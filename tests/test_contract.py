#!/usr/bin/env python3
"""
Contract Tests for template-operator-aget
...
class TestOperatorArchetype:
    def test_archetype_is_operator(self):
        archetype = get_template_root() / ".aget" / "persona" / "archetype.yaml"
        if archetype.exists():
            assert "operator" in archetype.read_text().lower()

    def test_governance_is_rigorous(self):
        """Operator must have rigorous governance."""
        archetype = get_template_root() / ".aget" / "persona" / "archetype.yaml"
        if archetype.exists():
            content = archetype.read_text().lower()
            assert "rigorous" in content
